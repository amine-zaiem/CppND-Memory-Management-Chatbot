...
const std::string chatbot_image_path = "../images/chatbot.png";

ChatLogic::ChatLogic()
{

    // create instance of chatbot
    _chatBot = new ChatBot(chatbot_image_path);
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        if (newNode == _nodes_ptr.end())
                        {
                            _nodes_ptr.emplace_back(std::make_unique<GraphNode>(id));
                            newNode = _nodes_ptr.end() - 1; // get iterator to last element
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(_nodes_ptr.begin(), _nodes_ptr.end(), [&parentToken](std::unique_ptr<GraphNode> &node) { return node->GetID() == std::stoi(parentToken->second); });
                            auto childNode = std::find_if(_nodes_ptr.begin(), _nodes_ptr.end(), [&childToken](std::unique_ptr<GraphNode> &node) { return node->GetID() == std::stoi(childToken->second); });
    ...
    ChatBot chatBot( chatbot_image_path );
